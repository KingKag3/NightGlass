...
A self-hosted, single-file cyber threat analysis platform. Ingest heterogeneous threat data (JSON / STIX-lite / CSV / raw text), visualize it as an interactive force-directed **threat constellation**, enforce **classification markings with clearance-based redaction**, run **automatic ingest pipelines** with translation, and fire **alerts from saved searches**. Zero build step, zero dependencies — open `index.html` and it works.
...
- **Classification & redaction** — U / CUI / SECRET / TOP SECRET markings with a proper banner bar; toggle globally, set per instance, per ingest batch, or per item; content above the active analyst's clearance renders blurred/redacted everywhere (graph, feed, timeline, matrix, search, inspector).
- **Custom tagging** — free-form tags on any entity, editable in the inspector, queryable everywhere.
...
This is an analysis tool for threat *data about* adversaries; it performs no scanning, exploitation, or contact with hostile infrastructure. Clearance-based redaction here is a client-side UX control — for real multi-user enforcement it must move server-side (see roadmap). Demo data is entirely fictional.
