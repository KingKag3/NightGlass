...
13. **[[Graph Regions]]** (backlog, captured 2026-07-15) — a labeled, colored boundary drawn around a cluster of nodes on the constellation graph, à la link-analysis "frames." New `state.regions[]` (explicit member entities, not tag-reuse — avoids inheriting the classification/markings opt-in gate); form-based membership first, canvas lasso-select as a distinct larger follow-up. Named "Regions," not "Groups," to avoid colliding with the existing campaign-Groups terminology.
14. **[[Status Badges]]** (backlog, captured 2026-07-15) — small config-driven status badges on an entity (deceased, BOLO, incarcerated...), same pattern as `TYPES`/`SEV`/`CLASSIF`. Distinct from `meta{}` (invisible, unstructured), `tags[]` (wrong opt-in gate), and `state.flags[]` (wrong semantics — follow-up, not permanent status). Touches three separate node-rendering surfaces (main canvas, profile radial preview, sandbox SVG) to show up consistently everywhere.
