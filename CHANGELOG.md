...
## v0.11.8 — 2026-07-15
- **Status badges** — a small, config-driven vocabulary of status flags for an entity (BOLO / Wanted, Deceased, Incarcerated, Relocated, Informant / Source), same pattern as `TYPES`/`SEV`/`CLASSIF` with a free-text escape hatch for anything not in the default list. New "Status" section on the Threat Actors profile (deliberately *not* gated behind the classification/markings opt-in toggle, unlike Tags — a status badge is basic enough it shouldn't require opting into advanced mode) — add by typing/picking a label, with an optional note, each badge dated and attributed (`setAt`/`setBy`); duplicate statuses are rejected with a toast rather than silently stacking. Badges render as an icon on the profile's radial local-neighborhood preview, as a small corner icon on the main constellation graph's nodes, and as a `status:`/`badge:` search operator (distinct from the existing `flagged:` operator, which means an open Investigators follow-up flag, not a status). Round-trips losslessly through JSON export/import and appears in the Markdown report. Deliberately scoped out of this pass: the sandbox mind-map's SVG nodes don't render badges yet — a draft workspace isn't where you'd typically be checking an established fact like "is this person deceased," so it's a fast-follow rather than a blocker.

## v0.11.7 — 2026-07-15
- **Sandbox mind-map now supports pan, zoom, and an expanded canvas**, so a large diagram doesn't outgrow the panel. Scroll to zoom toward the cursor (or use the new +/−/reset controls in a small floating toolbar), drag empty canvas to pan, and click "⤢ Expand" to grow the panel to ~72vh. Implementation mirrors the main constellation's `view{x,y,k}`/`toScreen`/`toWorld` camera pattern: a `sandboxView{tx,ty,k}` transform wraps the mind-map's nodes/edges in one `<g>`, so member `x`/`y` stay in a fixed "world" space untouched by the camera — dragging a node, hit-testing the link-handle drop target, and the typo-guard all keep working correctly at any zoom level. Expanding doesn't just grow the CSS box; the logical canvas (viewBox) grows to match the panel's actual measured aspect ratio (re-measured on toggle and window resize), so the extra screen space shows more diagram instead of empty letterboxing.
