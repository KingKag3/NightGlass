...
- **New CSS gate**: `.advonly` class + `body:not(.marked) .advonly{display:none}` (next to the existing `.classbanner`/`.marked` rule at the top of `<style>`). Applied to: the header's `.userbox` (active-analyst select + clearance chip — clearance is meaningless with markings off), the instance-classification select, the ingest form's classification/tag fields, the pipeline form's classification/tag fields, and the inspector's Classification + Tags `insp__section` blocks. Tag chips in search results and the pipeline list are now conditionally rendered on `markingsOn()` in JS instead (template strings, not static markup).
