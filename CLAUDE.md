...
| Force graph | `physicsStep`/`physics`/`organizeLayout` `draw` `tick` `nodeAt`/`linkAt`; camera `toScreen`/`toWorld`; `resizeCanvas`/`centerView`; pinning/hiding via `entity.pinned`/`entity.hidden` (view-state only, not exported); edge hover/click tooltip via `#edgeTip`; actor focus via `actorComponent`/`focusActor`/`renderActorsPanel` |
| Inspector drawer | `selectEntity` (also hosts classification + tag editors), `closeInspector` |
| Threat Actors view | `renderActorsView` (filterable list), `selectActorProfile`/`renderActorProfile` (editable profile — label, severity, meta, classification, tags, "Threat Group / Campaign" section listing linked `campaign` entities with a focus button), `handleImagesAdd` (multi-file → capped-size JPEG data URIs appended to `entity.images[]`, gallery UI), `drawProfilePreview`/`resizeProfilePreview`/`profilePreviewNodeAt`/`updatePreviewHeader` (radial 1-hop local-neighborhood preview, `previewFocusId` state for click-to-recenter); `profileActorId` state, distinct from `selectedId`/`focusedActorId`/`previewFocusId` |
...
v0.9.0. Implemented: constellation graph, command center, ATT&CK matrix, timeline, multi-format ingestion, auto-ingest pipelines (simulator + URL poll), translation (endpoint + demo dict), U/CUI/S/TS markings + clearance redaction across every view, custom tagging, saved searches + alert engine, JSON + Markdown export, relationship-type edge coloring with hover/click tooltips, node pinning/hiding, one-click layout organize, per-actor graph focus, a dedicated Threat Actors view with fully editable profiles (multi-image gallery, meta, campaign-level focus, radial local-neighborhood constellation preview). Boots into the Operation Nightjar demo with one alerting search and one paused simulator pipeline.
