...
| Attribute-key coloring | `attrColor(key)` — same deterministic-hash trick as `relColor` (own palette `ATTRPALETTE`, different mix constant so a key and a same-named relation don't coincidentally share a color); every meta key gets one stable color for free, no per-key config. Used for the small dot + key label in the inspector's Attributes `.kv` rows and the profile's editable `.metarow` key column. |
