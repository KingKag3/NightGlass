...
| Threat Actors view | `renderActorsView` (filterable list), `selectActorProfile`/`renderActorProfile` (editable profile — label, severity, meta, classification, tags, "Threat Group / Campaign" section listing linked `campaign` entities with a focus button), `handleImagesAdd` (multi-file → capped-size JPEG data URIs appended to `entity.images[]`, gallery UI), `drawProfilePreview`/`resizeProfilePreview`/`profilePreviewNodeAt`/`profilePreviewLinkAt`/`updatePreviewHeader`/`rootCampaignFor` (radial 1-hop local-neighborhood preview: full breadcrumb trail via `previewTrail` array, pulse animation via `previewPulses` driven by `tick()`'s `drawProfilePreview(true)` when this view is active, edge hover via `previewHoverLink`/`#profileEdgeTip` mirroring the main graph's `linkAt`/`#edgeTip`); `profileActorId` state, distinct from `selectedId`/`focusedActorId`/`previewTrail` |
