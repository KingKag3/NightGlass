...
| Threat Actors view | `renderActorsView` (filterable list), `selectActorProfile`/`renderActorProfile` (editable profile — label, severity, meta, classification, tags, "Threat Group / Campaign" section listing linked `campaign` entities with a focus button), `handleAvatarUpload` (image → capped-size JPEG data URI in `entity.image`), `drawProfilePreview`/`resizeProfilePreview`/`profilePreviewNodeAt` (live constellation preview, piggybacks on the always-running `tick()`/`physics()` loop — no second simulation); `profileActorId` state, distinct from `selectedId`/`focusedActorId` |
