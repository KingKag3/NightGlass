...
| Force graph | `physicsStep`/`physics`/`organizeLayout` `draw` `tick` `nodeAt`/`linkAt`; camera `toScreen`/`toWorld`; `resizeCanvas`/`centerView`; pinning/hiding via `entity.pinned`/`entity.hidden` (view-state only, not exported); edge hover/click tooltip via `#edgeTip`; actor focus via `actorComponent`/`focusActor`/`renderActorsPanel`; `actorsPanelGroupFilter` state narrows `renderActorsPanel`'s listed rows to one campaign's actors when 2+ campaigns are present |
...
v0.9.7. Implemented: constellation graph, command center (now with a "Critical Attention" pulsing watchlist card for critical-severity entities of any type), ATT&CK matrix, timeline, multi-format ingestion, auto-ingest pipelines (simulator + URL poll), translation (endpoint + demo dict), U/CUI/S/TS markings + clearance redaction across every view, custom tagging, saved searches + alert engine, JSON + Markdown export, relationship-type edge coloring with hover/click tooltips, collision-avoiding attribute-key coloring (`attrColor`/`attrColorsForKeys`, full hue wheel not a fixed palette) in the inspector and profile Attributes sections plus a themed/color-coded severity dropdown in the profile, node pinning/hiding, one-click layout organize, per-actor graph focus with a campaign group filter, a dedicated Threat Actors view (Groups + Actors sidebar, campaigns profile like actors) with fully editable profiles (multi-image gallery, meta, campaign-level focus, radial local-neighborhood constellation preview with a full breadcrumb trail, pulse animation, and edge hover tooltips matching the main graph). Boots into the Operation Nightjar demo with one alerting search and one paused simulator pipeline.
