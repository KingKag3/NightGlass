...
| Force graph | `physicsStep`/`physics`/`organizeLayout` `draw` `tick` `nodeAt`/`linkAt`; camera `toScreen`/`toWorld`; `resizeCanvas`/`centerView`; pinning/hiding via `entity.pinned`/`entity.hidden` (view-state only, not exported); edge hover/click tooltip via `#edgeTip` |
