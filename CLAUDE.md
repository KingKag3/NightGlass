...
| Threat Actors view | `renderActorsView` (filterable list), `selectActorProfile`/`renderActorProfile` (editable profile — label, severity, meta, classification, tags, "Threat Group / Campaign" section listing linked `campaign` entities with a focus button), `handleImagesAdd` (multi-file → capped-size JPEG data URIs appended to `entity.images[]`, gallery UI), `drawProfilePreview`/`resizeProfilePreview`/`profilePreviewNodeAt`/`updatePreviewHeader`/`rootCampaignFor` (radial 1-hop local-neighborhood preview with a full breadcrumb trail, `previewTrail` array state for click-to-recenter/crumb-truncate); `profileActorId` state, distinct from `selectedId`/`focusedActorId`/`previewTrail` |
