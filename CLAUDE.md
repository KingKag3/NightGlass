...
| Threat Actors view | `renderActorsView` (filterable list), `selectActorProfile`/`renderActorProfile` (editable profile — label, severity, meta, classification, tags), `handleAvatarUpload` (image → capped-size JPEG data URI in `entity.image`); `profileActorId` state, distinct from `selectedId`/`focusedActorId` |
